...
정적 GitHub Pages는 문자를 직접 보내지 못합니다. `sms-reminder` 워크플로가 **매일 10:00·13:00 KST**에 일정을 확인하고, **서울 원정 일반예매** 오픈 1시간 전부터 오픈 직전 사이에만 발송합니다. 홈경기·서울 밖 원정 창은 건너뜁니다.

- **10:00 KST** → LG·두산 일반예매(11:00 오픈) 1시간 전
- **13:00 KST** → 키움 일반예매(14:00 오픈) 1시간 전
...
이 선택은 `sms.config.json`의 `enabled`에 저장되어 이후 매일 10:00·13:00 KST 실행에 적용됩니다. 같은 실행에서 홈페이지 ON/OFF 표시(Pages 폴백)도 함께 맞춥니다.
